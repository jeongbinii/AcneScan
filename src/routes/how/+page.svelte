...
<div class="min-h-screen bg-gray-50" style="background-image: radial-gradient(circle, #e5e7eb 1px, transparent 1px); background-size: 24px 24px;">
	<header class="border-b border-gray-200/60 bg-white/80 backdrop-blur-md">
		<div class="mx-auto flex max-w-6xl items-center justify-between px-6 py-4">
			...
			<nav class="flex items-center gap-5 text-sm text-gray-500">
				<a href="/how" class="text-gray-900 font-medium">이용 방법</a>
	...
	<main class="mx-auto max-w-4xl px-6 py-16">
		<div class="mb-12 text-center">
			...
			<h1 class="mb-3 text-4xl font-black text-gray-900">이용 방법</h1>
			<p class="text-gray-500">간단한 3단계로 여드름을 분석하고 관리해보세요.</p>
		</div>

		<div class="space-y-6">
			{#each [
				{ icon: '📷', step: '01', title: '사진 업로드', desc: '여드름 부위를 촬영하거나 갤러리에서 사진을 선택하세요.', detail: '얼굴 전체가 아닌, 여드름이 있는 부위를 가까이서 촬영하면 더 정확한 분석이 가능합니다. JPG, PNG, WEBP 형식을 지원해요.' },
				{ icon: '🔬', step: '02', title: 'AI 분석', desc: 'AI가 여드름 종류와 심각도를 즉시 분석합니다.', detail: '업로드한 사진을 AI가 분석하여 여드름 종류(화농성, 구진성, 블랙헤드 등)와 심각도(경증/중등증/중증)를 판별합니다. 약 2~3초면 결과를 확인할 수 있어요.' },
				{ icon: '💊', step: '03', title: '맞춤 솔루션 확인', desc: '약국에서 구할 수 있는 제품으로 맞춤 케어 방법을 제안해드려요.', detail: '분석 결과에 따라 연고, 패치, 세안제 등 시중에서 쉽게 구할 수 있는 아이템을 조합한 해결방안을 제시합니다. 마이홈에 보유 아이템을 등록해두면 AI가 이를 활용한 맞춤 방안을 제안해줘요.' }
			] as item}
				<div class="relative flex gap-6 rounded-2xl border border-gray-200 bg-white p-8 shadow-sm">
		...
		<div class="mt-16">
			<div class="mb-6 text-center">
				<p class="mb-2 text-xs font-bold uppercase tracking-widest text-violet-500">Subscription</p>
				<h2 class="text-2xl font-black text-gray-900">구독 사용자라면?</h2>
			</div>
			<div class="space-y-4">
				{#each [
		...
		<div class="mt-12 text-center">
			<a href="/subscribe" class="inline-block rounded-xl bg-gray-900 px-8 py-3 font-semibold text-white hover:bg-gray-800 transition-colors">
				구독하기
			</a>
		</div>
	</main>

	<footer class="border-t border-gray-200 bg-white py-8 text-center text-sm text-gray-400">
